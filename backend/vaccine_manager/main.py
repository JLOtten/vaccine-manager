...
@app.get("/users/{user_id}/families/{family_id}/family_members", 
...
@app.post("/users/{user_id}/families/{family_id}/family_members/{family_member_id}/vaccine_records", 
...
def create_vaccine_record(
    ...
    db_vaccine_record = models.VaccineRecord(family_member_id=family_member_id, 
                                             location=record.location, dosage=record.dosage, 
                                             vaccine_id=record.vaccine_id, date=record.date)
    db.add(db_vaccine_record)
    db.commit()
    db.refresh(db_vaccine_record)
    return db_vaccine_record

@app.get("/users/{user_id}/families/{family_id}/family_members/{family_member_id}/vaccine_records", 
         response_model=List[pydantic_models.VaccineRecord])
def get_vaccine_record(user_id: int, family_id: int, family_member_id: int, db:Session = Depends(get_db)):
    return db.query(models.VaccineRecord).filter_by(family_member_id=family_member_id).all()
